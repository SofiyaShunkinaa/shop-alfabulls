...
<script type="text/javascript">
    ...
    function initPvzMap(pvzList) {
        let selectedCity = $('#adresserch').val();
        let firstAddress = Object.values(pvzList)[0];
        console.log(pvzList);
        
        ymaps.geocode(`${selectedCity}, ${firstAddress}`, {}).then(function (res) {
            ...
            for (let key in pvzList) {
                ...
                ymaps.geocode(address).then(function (res) {
                    ...
                    placemark.events.add('click', function (e) {
                        $('#pvz_curs').html('Ваш пвз по адресу: ' + address);
                        $('#selected-pvz').val(address);
                        $('#adressCode').val(pvzCode);
