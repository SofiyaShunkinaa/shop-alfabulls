...
<div class="dublicates">
        ...
        <div class="dublicates_box">
            ...
            <div class="flex_dovl_flex">
                @for($i = 0; $i < 6; $i++)
                        @php
                            $image = $dog->images[$i] ?? null;
                            //var_dump($image);
                            $imagePath = $image ? $image->path : null;
                        @endphp

                        <div>
                            <label class='felican {{ $imagePath ? "activ_image" : "" }}'>
                                <input class='felican2' type="file" name="images[]">

                                @if($imagePath)
                                    {{-- Передаём ID, чтобы не удалять это изображение --}}
                                    <input type="hidden" name="existing_images[]" value="{{ $image->id }}">
                                    <img src="{{ asset('storage/' . $imagePath) }}" alt="Фото щенка">
                                @else
                                    <img src="/img/dovn.png" alt="Пусто">
                                @endif

                                <img class='dizlaik' src="/img/del.png" alt="Удалить">
                            </label>
                        </div>
                @endfor
            </div>
            ...
            <div class="flex_dovl_flex">
                @for($i = 0; $i < 6; $i++)
                        ...
                        <div>
                            <label class='felican {{ $videoPath ? "activ_image" : "" }}'>
                                <input class='felican2' type="file" name="videos[]">

                                @if($videoPath)
                                    {{-- Передаём ID, чтобы не удалять это видео --}}
                                    <input type="hidden" name="existing_videos[]" value="{{ $video->id }}">
                                     {{-- Превью видео (маленькое) --}}
                                    <video width="100" height="auto"
                                        preload="metadata"
                                        class="video-thumb"
                                        data-video="{{ asset('storage/' . $videoPath) }}"
    ...
    <div class='flex_form_profuct'>
        ...
        <div class="form-group_flex">
            <p>Порода</p>             
            <input type="text" name="type" class='willi'placeholder="Порода"
                   required maxlength="100" value="{{ old('type') ?? $dog->type ?? '' }}">
        </div>
        <input type="hidden" name="slug" value="{{ old('slug') ?? $dog->slug ?? '' }}">


            <div class="flex_input">
                <div><p>Дата рождения</p> <input type="text"  name="date"
           required maxlength="100" value="{{ old('date') ?? $dog->date ?? '' }}"></div>
                <div><p>Цена</p> <input type="text" name="pric"
           required maxlength="100" value="{{ old('pric') ?? $dog->pric ?? '' }}"></div>
                <div><p>Старая цена</p> <input type="text" name="oldprice"
           value="{{ old('oldprice') ?? $dog->oldprice ?? '' }}"></div>

                <div><p>Скидка</p> <input type="text" name="sale"
           value="{{ old('sale') ?? $dog->sale ?? '' }}"></div>
                <div><p>Себестоимость</p> <input type="text" name="baseprice"
           value="{{ old('baseprice') ?? $dog->baseprice ?? '' }}"></div>
            </div>
    </div>
 ...
 <textarea name="opis" placeholder="Описание"
...
<div id="videoModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; 
    background:rgba(0,0,0,0.8); justify-content:center; align-items:center; z-index: 11111111111;">
    <button type="button" class="modal-close" onclick="closeModal()">&times;</button>
    <video id="modalVideo" controls style="max-width:90%; max-height:90%;">
...
function renderPreviews() {
    ...
    $(document).on('click', '.activ_image .dizlaik', function (event) {
        ...
        const container = $(this).closest('.felican');

        // Если есть <video> — заменяем его на <img>
        ...
        if (video.length) {
            ...
            video.replaceWith('<img src="/img/dovn.png">');
        } else {
            // Если было изображение — просто сбрасываем src
            container.find('img').first().attr('src', '/img/dovn.png');
        }

        // Удаляем класс
        container.removeClass('activ_image');

        // Удаляем скрытые inputs
        container.find('input[type="hidden"][name^="existing_images"]').remove();
        container.find('input[type="hidden"][name^="existing_videos"]').remove();
    });
</script>
