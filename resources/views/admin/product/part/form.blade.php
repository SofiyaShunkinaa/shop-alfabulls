 ...
 <div class='flex_form_profuct'>
    ...
    <div class="form-group_flex">
        ...
        <div class="flex_sticer">
            <label>
            <div class="{{ ($product->hit === 1) ? 'activ' : '' }}">

                <input type="checkbox" name="hit" value="1" {{ ($product->hit === 1) ? 'checked' : '' }}>
                <img src="/img/stik1.png" alt="hit">
            </div>
            </label>
            <label>
            <div class="{{ ($product->sale === 1) ? 'activ' : '' }}">
                <input type="checkbox" name="sale" value="1" {{ ($product->sale === 1) ? 'checked' : '' }}>
                <img src="/img/stik2.png" alt="Sale">
            </div>
            </label>
             <label>
            <div class="{{ ($product->new === 1) ? 'activ' : '' }}">
                <input type="checkbox" name="new" value="1" {{ old('new', $product->new ?? 0) ? 'checked' : '' }}>
...
document.querySelectorAll('input[name="category_id"]').forEach(radio => {
    ...
    $(document).on('click', '.activ_image .dizlaik' , function (event){
              event.preventDefault();
               $(this).parent().find('img').first().attr('src', "/img/dovn.png");
             $(this).parent().removeClass('activ_image');
...
function renderPreviews() {
   ...
   <script id="dropdownToggle">
      $('.flex_sticer input[type="checkbox"]').on('change', function() {
            $(this).parent().toggleClass("activ", this.checked);
        });
    ...
    `);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const checkboxes = document.querySelectorAll('#selectedCategory input[type="checkbox"]');
