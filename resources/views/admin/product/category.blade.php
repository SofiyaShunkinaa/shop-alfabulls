...
@php
    $currentCategoryId = request()->route('category')->id;
@endphp

@section('content')
    <h1 class="dog-h1 mb-4">{{ $category->name }}</h1>
    @if (count($products))
        ...
        <div class="mb-3">
            ...
            <a href="{{ route('admin.product.category', ['category' => $currentCategoryId]) }}" class="butn {{ $showArchived ? 'active' : '' }}"
            style="background-color: {{ $showArchived ? '#fff' : '#A6A6A6' }}; color: {{ $showArchived ? '#505050' : '#FFFFFF' }}; border-radius: 10px;">
                Активные
            </a>
            <a href="{{ route('admin.product.category', ['category' => $currentCategoryId, 'archived' => 0]) }}" class="butn {{ !$showArchived ? 'active' : '' }}"
        ...
        <div style="width: max-content;">
            ...
            <div class="d-flex gap-2 pb-3 mb-1" style="overflow: auto;">
                @foreach ($roots as $root)
                <a href="{{ route('admin.product.category', ['category' => $root->id]) }}" class="category-btn {{ $root->id == $currentCategoryId ? 'active' : '' }}">
                    {{ $root->name }}
                </a>
                @endforeach
            </div>
...
<script>
    ...
    const SHOW_ARCHIVED = {{ $showArchived ? 1 : 0 }};
    ...
    document.addEventListener('click', function (e) {
        ...
        // Проверяем, чтобы ссылка шла на пагинацию
        const urlObj = new URL(link.href, window.location.origin);
        const isPagination = urlObj.searchParams.has('page');
        ...
        urlObj.searchParams.set('archived', SHOW_ARCHIVED);
        urlObj.searchParams.set('category_id', CURRENT_CATEGORY_ID);
        if (query) urlObj.searchParams.set('name', query);
        ...
        .catch(err => console.error('Ошибка при пагинации:', err));
    });
</script>
...
document.getElementById('product-search').addEventListener('input', function () {
    ...
    debounceTimeout = setTimeout(() => {
        ...
        let url = `/admin/product/search?archived=${showArchived}`;
        if (query.length) {
